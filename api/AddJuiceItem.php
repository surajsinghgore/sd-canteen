...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['SendJuiceItem'])) {
        ...
        if (strlen($_FILES['JuiceImage']['name']) == 0) {
            $error_status = "warn";
            $error_message = 'Please Enter Juice Image';
            $toast_status = 'true';
            return;
        }
        ...
        if (isset($_REQUEST['orderStatus'])) {
            ...
            if (($_FILES['JuiceImage']['type'] == "image/png") || ($_FILES['JuiceImage']['type'] == "image/jpg") || ($_FILES['JuiceImage']['type'] == "image/jpeg") || ($_FILES['JuiceImage']['type'] == "image/webp")) {
                ...
                if ($sizeInMb > 5) {
                    ...
                    $resultGet = mysqli_query($connection, "SELECT * FROM juiceitems where juicename ='$JuiceName'");
                    ...
                    if ($rows > 0) {
                        ...
                        if (move_uploaded_file($tmp_name, $newLocationName)) {
                            ...
                            $sql_query = "insert into juiceitems(juicename,qty,category,active,normalprice,smallprice,mediumprice,largeprice,description,imagepath) values('$JuiceName',$JuiceQty,'$JuiceCategory','on',$normalPrice,$smallPrice,$mediumPrice,$largePrice,'$description','$newLocationName')";

                            $res = mysqli_query($connection, $sql_query);
                            if ($res) {

                                $error_status = "success";
                                $error_message = "$JuiceName Successfully Added";
                                $toast_status = 'true';
                                header('Location: /sd-canteen/admin/viewjuiceItem.php');
                            }
                            // failed to send
                            else {
                ...
                if ($sizeInMb > 5) {
                    ...
                    if ($rows > 0) {
                        ...
                        if (move_uploaded_file($tmp_name, $newLocationName)) {
                            ...
                            $sql_query = "insert into juiceitems(juicename,qty,category,active,normalprice,smallprice,mediumprice,largeprice,description,imagepath) values('$JuiceName',$JuiceQty,'$JuiceCategory','off',$normalPrice,$smallPrice,$mediumPrice,$largePrice,'$description','$newLocationName')";
