...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($foodId)) {
        ...
        $resultGet = mysqli_query($connection, "SELECT * FROM juiceitems where id=$foodId");
        ...
        if ($foodItemFind == 0) {
            $error_status = "error";
            $error_message = "Juice Item with this Id Not Found";
            $toast_status = 'true';
            return;
        } else {
            ...
            if (unlink($imagepath)) {
                $sql_query = "delete from juiceitems where id=$foodId";
                $deleteFoodRess = mysqli_query($connection, $sql_query);
                if ($deleteFoodRess) {
                    $error_status = "success";
                    $error_message = "Juice Item Successfully deleted";
                    $toast_status = 'true';
