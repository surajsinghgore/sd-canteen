...
if (isset($_REQUEST['loadPage'])) {
    ...
    $food_item_count = "select * from drinkitems where active='on'";
...
<?php
 ...
 <li  onclick='loadParticularCategory("<?php echo $allCategoryData['drinkcategoryname'];?>")'><span class="heading">
    ...
    <?php echo $allCategoryData['drinkcategoryname'];?></span>

    <!-- count food in category -->
    <span class="length"> <?php 
    $countLength = "select * from drinkitems where category='".$allCategoryData['drinkcategoryname']."' and active='on'";
...
$food_item_count = "select * from drinkitems where active='on' and category like '$category'";
$res_food_item_count = mysqli_query($connection, $food_item_count);
$FoodItemsCount = mysqli_num_rows($res_food_item_count);
...
if($FoodItemsCount>0){
    ...
    onclick='removeFromCart("<?php echo $allFoodData['id'];?>","<?php echo $allFoodData['drinkname'];?>")'
    >Remove Item</button>
    
    <!-- add to cart -->
    <button onclick='addToCart("<?php echo $allFoodData['id'];?>","<?php echo $allFoodData['drinkname'];?>")' id="addToCartBtn<?php echo $allFoodData['id'];?>">Add To Cart</button>
    
    <!-- buy now button -->
    <button class="buy" onclick='addToCartAndBuy("<?php echo $allFoodData['id'];?>","<?php echo $allFoodData['drinkname'];?>")'>Buy Now</button>
    </div>
